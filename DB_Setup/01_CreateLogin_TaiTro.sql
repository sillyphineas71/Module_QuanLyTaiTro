...
/* --- 1. LOGIN cấp server ------------------------------------------------------------------ */
IF NOT EXISTS (SELECT 1 FROM sys.server_principals WHERE name = N'TT_APP_LOGIN')
BEGIN
    CREATE LOGIN [TT_APP_LOGIN]
        WITH PASSWORD = N'<<LEAD_DIEN_MAT_KHAU_MANH>>',
             CHECK_POLICY = ON,
             DEFAULT_DATABASE = [ESS_HOCVIENTAICHINH_DAOTAO];
END
GO
...
GRANT SELECT ON dbo.CSV_TaiKhoan      TO [TT_APP_USER];
GRANT SELECT ON dbo.CSV_ThongTin      TO [TT_APP_USER];
GRANT SELECT ON dbo.STU_Lop           TO [TT_APP_USER];
GRANT SELECT ON dbo.dmHe              TO [TT_APP_USER];
GRANT SELECT ON dbo.dmKhoa            TO [TT_APP_USER];
GRANT SELECT ON dbo.dmChuyenNganh     TO [TT_APP_USER];
GO
...
GRANT SELECT ON dbo.STU_HoSoSinhVien (Ho_ten, Ngay_sinh) TO [TT_APP_USER];
GO
...
SELECT @sql = @sql + N'GRANT SELECT, INSERT, UPDATE, DELETE ON dbo.'
                   + QUOTENAME(name) + N' TO [TT_APP_USER];' + CHAR(10)
FROM   sys.tables
WHERE  name LIKE 'TT[_]%' AND SCHEMA_NAME(schema_id) = 'dbo'
...
BEGIN
    GRANT SELECT, DELETE ON dbo.TT_MaDoiPhien TO [TT_APP_USER];
    DENY  INSERT, UPDATE ON dbo.TT_MaDoiPhien TO [TT_APP_USER];
END

IF OBJECT_ID('dbo.TT_MaDoiPhien_Tao', 'P') IS NOT NULL
    DENY EXECUTE ON dbo.TT_MaDoiPhien_Tao TO [TT_APP_USER];
GO
